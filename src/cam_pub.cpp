...
CamPub::CamPub(bool init) : Node("cam_pub")
{
    ...
    this->declare_parameter<std::vector<int64_t>>("image_size", std::vector<int64_t>{640, 480});
    ...
    this->set_camera_topic(
        this->get_camera_name() + this->get_parameter("camera_topic").as_string());
    this->set_camera_status_topic(
        this->get_camera_name() + this->get_parameter("camera_status_topic").as_string());
    ...
    try
    {
        RCLCPP_INFO(this->get_logger(), "Initializing camera at %s", this->get_device_path().c_str());

        // Open video capture
        this->cap_.open(this->get_device_path());
        if (!this->cap_.isOpened())
        {
            RCLCPP_ERROR(this->get_logger(), "Cannot open camera at %s", this->get_device_path().c_str());
            this->set_is_publishing(false);
        }
        else
        {
            // Set image size
            this->cap_.set(cv::CAP_PROP_FRAME_WIDTH, static_cast<double>(this->get_image_size()[0]));
            this->cap_.set(cv::CAP_PROP_FRAME_HEIGHT, static_cast<double>(this->get_image_size()[1]));
            RCLCPP_INFO(this->get_logger(), "Camera opened: %s", this->get_device_path().c_str());
...
// Getters
const std::string &CamPub::get_camera_name() const
{
    return this->cam_name_;
}
...
int CamPub::get_publish_rate() const
{
...
bool CamPub::is_publishing() const
{
    return this->is_publishing_;
}

// Methods
bool CamPub::capture_frame(cv::Mat &frame)
{
...
void CamPub::initialize()
{
    ...
    this->timer_ = this->create_wall_timer(fps, std::bind(&CamPub::timer_callback, this));
}
